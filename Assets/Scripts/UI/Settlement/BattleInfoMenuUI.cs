...
public class BattleInfoMenuUI : MonoBehaviour
{
    ...
    [SerializeField] public bool relativeMode = false;
    ...
    public void InitializeMenu()
    {
        ...
        if (relativeMode)
        {
            foreach (var player in currentLobby.Players)
            {
                ...
                Transform parentList = playerTeam == localTeam ? blueTeamList : orangeTeamList;
                PlayerInfoUI info = Instantiate(playerPrefab, parentList).GetComponent<PlayerInfoUI>();
                info.SetPlayerInfo(player, playerInfoType, relativeMode);
            }
        }
        else
        {
